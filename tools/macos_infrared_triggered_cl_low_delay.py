...
def cmd_capture_triggered_dual_combined_low_delay(args: argparse.Namespace) -> int:
    ...
    with contextlib.ExitStack() as stack:
        hid = import_hid()
        trigger_device = hid_open_by_args(hid, args.trigger_hid_path, args.trigger_vid, args.trigger_pid)
        stack.callback(trigger_device.close)
        trigger_waiter = Ch9326GpioPollTriggerWaiter(
            trigger_device,
            args.gpio_report_id,
            args.gpio_report_length,
            condition,
        ...
        metadata = {
            ...
            "trigger": {
                "io": condition.io,
                "edge": condition.edge,
                "byteIndex": condition.byte_index,
                "mask": condition.mask,
                "activeValue": condition.active_value,
                "captureSeconds": args.capture_seconds,
                "gpioReportId": args.gpio_report_id,
                "gpioReportLength": args.gpio_report_length,
                "gpioPollIntervalSeconds": args.gpio_poll_interval,
            },
            "mlxAddress": args.address,
            "mlxBaud": args.mlx_baud,
            "mlxTimeoutSeconds": args.mlx_timeout,
                        ...
                        try:
                            ...
                            while True:
                                if args.trigger_count and trigger_count >= args.trigger_count:
                                    break
                                print("Waiting for platform trigger...")
                                report = trigger_waiter.wait_for_trigger()
                                trigger_count += 1
                                ...
                                if args.trigger_cooldown > 0:
                                    time.sleep(args.trigger_cooldown)
                                if args.rearm_wait_inactive and condition.edge != "any-report":
                                    ok = trigger_waiter.wait_for_rearm_ready(args.rearm_timeout)
                                    if ok:
                                        print("Trigger input returned to re-arm state; re-armed.")
                                    else:
                                        print("Trigger input did not return to re-arm state before timeout; re-arming anyway.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    capture_triggered_dual_low_delay = subparsers.add_parser(
        ...
        help="Initialize MLX/TA612 once, continuously capture, and write 1s trigger windows with low IO-to-record latency",
    )
    add_ch9326_trigger_args(capture_triggered_dual_low_delay, default_edge="falling")
    add_ch9326_gpio_poll_args(capture_triggered_dual_low_delay)
    ...
    capture_triggered_dual_low_delay.add_argument("--trigger-cooldown", type=float, default=0.2, help="Delay after each trigger before checking re-arm state")
    capture_triggered_dual_low_delay.add_argument(
        "--rearm-wait-inactive",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Before re-arming, wait until the trigger input returns to the opposite state required for the next edge",
    )
    capture_triggered_dual_low_delay.add_argument("--rearm-timeout", type=float, default=30.0, help="Maximum seconds to wait for trigger input to return to re-arm state")
