...
class TriggerWindowSummaryWriter:
    def __init__(self, session_dir: Path, include_mlx_channel: bool = False) -> None:
        self.include_mlx_channel = include_mlx_channel
        self._lock = threading.Lock()
    ...
    def __enter__(self) -> "TriggerWindowSummaryWriter":
        return self
    ...
    def add_event(
        ...
        start = to_east8(trigger_timestamp)
    ...
    def write(self, mlx: MlxFrameSummary, tasi: TasiSerialSample | None) -> None:
        ...
        with self._lock:
            matching = [window for window in self._windows if window["start"] <= timestamp <= window["end"]]
            if not matching:
                return
            rows = [self._build_row(window, mlx, tasi, timestamp) for window in matching]
            for row in rows:
                self._summary_csv.writerow(row)
    ...
    def _build_row(
        ...
        start = window["start"]
        trigger_index = int(window["trigger_index"])
        offset_ms = (timestamp - start).total_seconds() * 1000.0 if isinstance(start, datetime) else 0.0
        if tasi is not None and tasi.channels_c is not None:
            age_ms = (mlx.timestamp_utc - tasi.timestamp_utc).total_seconds() * 1000.0
            tasi_timestamp = east8_iso(tasi.timestamp_utc)
            channels = [f"{value:.6f}" for value in tasi.channels_c]
...
class MlxChannelCaptureWorker:
    def __init__(
        ...
        trigger_window_writer: TriggerWindowSummaryWriter | None = None,
        quiet_live: bool = False,
    ) -> None:
        ...
        self.trigger_window_writer = trigger_window_writer
        self.quiet_live = quiet_live
        self.full_frame_gate = MlxFullFrameGate()
        self.timing_stats = MlxSubpageTimingStats(1.0 / refresh_rate_hz)
        self.frames = 0
        self.subpages = 0
        self._latest_lock = threading.Lock()
        self._latest_summary: MlxFrameSummary | None = None
        self._error: BaseException | None = None
        self._thread: threading.Thread | None = None
    ...
    def latest_summary(self) -> MlxFrameSummary | None:
    ...
    def _run(self) -> None:
        ...
        try:
            while not self.stop_event.is_set():
                ...
                if self.trigger_window_writer is not None:
                    self.trigger_window_writer.write(summary, latest_tasi)

                now = time.monotonic()
                if not self.quiet_live and (self.frames % self.print_every == 0 or now - last_rate_print >= 1.0):
                    elapsed = max(now - start, 1e-6)
                    ...
                    with self.print_lock:
                        print(
                            ...
                            f"Ta={summary.ta_c:.2f}C avg={summary.avg_c:.2f}C max={summary.max_c:.2f}C "
...
def format_trigger_temperature_summary(
    trigger_index: int,
    trigger_timestamp: datetime,
    workers: Sequence[MlxChannelCaptureWorker],
    tasi: TasiSerialSample | None,
) -> str:
    parts = [f"Trigger {trigger_index} {trigger_timestamp.isoformat()}"]
    for worker in workers:
        summary = worker.latest_summary()
        if summary is None:
            parts.append(f"{worker.channel}=no-mlx-frame-yet")
            continue
        age_ms = (trigger_timestamp - summary.timestamp_utc).total_seconds() * 1000.0
        parts.append(
            f"{worker.channel}:avg={summary.avg_c:.2f}C "
            f"max={summary.max_c:.2f}C "
            f"center={summary.center_c:.2f}C frame_age={age_ms:.0f}ms"
        )
    if tasi is not None and tasi.channels_c is not None:
        tasi_age_ms = (trigger_timestamp - tasi.timestamp_utc).total_seconds() * 1000.0
        channels = ",".join(f"{value:.1f}C" for value in tasi.channels_c)
        parts.append(f"TA612=[{channels}] age={tasi_age_ms:.0f}ms")
    else:
        parts.append("TA612=none")
    return " | ".join(parts)


def wait_for_tasi_threshold(
    tasi_poller: TasiSerialPoller,
    workers: Sequence[MlxChannelCaptureWorker],
    channel: int,
    ...
    if channel < 1 or channel > 4:
        raise CliError("--start-when-tasi-channel must be 1, 2, 3, or 4")
    if direction not in ("rising", "falling"):
        raise CliError("--start-when-tasi-direction must be rising or falling")
    if direction == "falling":
        print(f"Waiting for TA612-{channel} to fall through {threshold_c:.1f}C before auto triggers...")
    else:
        print(f"Waiting for TA612-{channel} >= {threshold_c:.1f}C before auto triggers...")
    start = time.monotonic()
    last_print = 0.0
    seen_above = False
    while True:
        ...
        for worker in workers:
            worker.check_error()
        sample = tasi_poller.latest()
        value = None
        if sample is not None and sample.channels_c is not None and len(sample.channels_c) >= channel:
            candidate = sample.channels_c[channel - 1]
        ...
        if value is not None:
            if value > threshold_c:
                seen_above = True
            if direction == "rising" and value >= threshold_c:
                print(f"TA612-{channel} reached {value:.1f}C; auto interval triggers start now.")
                return
            if direction == "falling" and seen_above and value <= threshold_c:
                print(f"TA612-{channel} fell to {value:.1f}C; auto interval triggers start now.")
                return
        if timeout_seconds > 0 and now - start >= timeout_seconds:
            latest_text = f"{value:.1f}C" if value is not None else "no valid sample"
            action_text = "fall through" if direction == "falling" else "reach"
            raise CliError(
                f"TA612-{channel} did not {action_text} {threshold_c:.1f}C within {timeout_seconds:.1f}s "
                f"(latest: {latest_text})"
            )
        if now - last_print >= 1.0:
            latest_text = f"{value:.1f}C" if value is not None else "waiting for valid sample"
            if direction == "falling":
                state_text = "seen above target" if seen_above else "waiting to see above target first"
                print(f"TA612-{channel} {latest_text}; target falling through {threshold_c:.1f}C; {state_text}")
            else:
                print(f"TA612-{channel} {latest_text}; target {threshold_c:.1f}C")
            last_print = now
        time.sleep(0.05)
...
def cmd_capture_triggered_dual_combined_low_delay(args: argparse.Namespace) -> int:
    left_port, right_port = resolve_dual_mlx_ports(args.left_mlx_port, args.right_mlx_port)
    tasi_port = args.tasi_port or select_default_tasi_port()
    capture_root = Path(args.capture_root)
    ...
    with contextlib.ExitStack() as stack:
        ...
        for channel, port, refresh_rate_hz in channel_specs:
            ...
            writer = stack.enter_context(
                MlxCaptureWriter(
                    capture_root,
                    {"kind": "mac_dual_mlx_tasi_low_delay_channel", "channel": channel},
                    eeprom,
        ...
        metadata = {
            "createdEast8": east8_now().isoformat(),
            "kind": "mac_dual_mlx_tasi_low_delay",
            "lowDelayMode": "devices_initialized_once_continuous_capture_trigger_windows",
            "trigger": {
                "source": "auto_interval",
                "intervalSeconds": args.trigger_interval,
                "edge": condition.edge,
                "captureSeconds": args.capture_seconds,
                "startWhenTasiChannel": args.start_when_tasi_channel,
                "startWhenTasiAboveC": args.start_when_tasi_above,
                "startWhenTasiDirection": args.start_when_tasi_direction,
                "startWhenTasiTimeoutSeconds": args.start_when_tasi_timeout,
            },
            "mlxAddress": args.address,
            "mlxBaud": args.mlx_baud,
            "mlxTimeoutSeconds": args.mlx_timeout,
            ...
            "tasiTimeoutSeconds": args.tasi_timeout,
            "tasiPollIntervalSeconds": args.tasi_poll_interval,
            "joinPolicy": "Continuous capture; trigger_window_summary.csv contains MLX frames whose timestamps fall inside trigger windows.",
            "mlxChannels": [
                {
                    "channel": str(item["channel"]),
                    "port": str(item["port"]),
        ...
        with TasiSerialCaptureWriter(session_dir) as tasi_writer:
            with JoinedSummaryWriter(session_dir, include_mlx_channel=True) as joined_writer:
                with TriggerWindowSummaryWriter(session_dir, include_mlx_channel=True) as trigger_writer:
                    with TasiSerialPoller(
                        tasi_port,
                        args.tasi_baud,
                        args.tasi_timeout,
                        ...
                        workers = [
                            MlxChannelCaptureWorker(
                                ...
                                trigger_window_writer=trigger_writer,
                                quiet_live=args.quiet_live,
                            )
                            for item in prepared
                        ]
                        print(f"Low-delay capture directory: {session_dir}")
                        print(f"MLX left:  {left_port} @ {args.mlx_baud}")
                        print(f"MLX right: {right_port} @ {args.mlx_baud}")
                        print(f"TA612:     {tasi_port} @ {args.tasi_baud}")
                        print(
                            "Auto-interval low-delay trigger armed. "
                            f"interval_seconds={args.trigger_interval}; "
                            f"window_seconds={args.capture_seconds}"
                        )
                        start = time.monotonic()
                        try:
                            for worker in workers:
                            ...
                            if args.warmup_seconds > 0:
                                ...
                                while time.monotonic() < warmup_deadline:
                                    ...
                                    for worker in workers:
                                        worker.check_error()
                            wait_for_tasi_threshold(
                                tasi_poller,
                                workers,
                                args.start_when_tasi_channel,
                                args.start_when_tasi_above,
                                args.start_when_tasi_timeout,
                                args.start_when_tasi_direction,
                            )
                            next_trigger_time = time.monotonic()
                            while True:
                                if args.trigger_count and trigger_count >= args.trigger_count:
                                    break
                                remaining = next_trigger_time - time.monotonic()
                                if remaining > 0:
                                    print(f"Waiting for auto trigger in {remaining:.3f}s...")
                                else:
                                    print("Auto trigger ready...")
                                while True:
                                    remaining = next_trigger_time - time.monotonic()
                                    if remaining <= 0:
                                        break
                                    ...
                                    for worker in workers:
                                        worker.check_error()
                                trigger_count += 1
                                trigger_timestamp = east8_now()
                                last_trigger_time = trigger_timestamp
                                report = f"auto-interval:{args.trigger_interval:.6f}".encode("ascii")
                                trigger_writer.add_event(
                                    trigger_count,
                                    trigger_timestamp,
                                    report,
                                    condition,
                                ...
                                if args.quiet_live:
                                    ...
                                    print(
                                        f"Trigger {trigger_count} at {trigger_timestamp.isoformat()} "
                                        f"window_end={(trigger_timestamp + timedelta(seconds=args.capture_seconds)).isoformat()} "
                                        f"report={hex_bytes(report, limit=32)}"
                                    )
                                tasi_poller.check_error()
                                for worker in workers:
                                    worker.check_error()
                                next_trigger_time = time.monotonic() + max(0.0, args.trigger_interval)
                        finally:
                            if last_trigger_time is not None:
                                remaining = (
                                    last_trigger_time + timedelta(seconds=args.capture_seconds) - east8_now()
                                ).total_seconds()
                                if remaining > 0:
                        ...
                        print(
                            f"Stopped low-delay capture. triggers={trigger_count}, {summary_text}, "
                            f"tasi_frames={tasi_poller.frames}, output={session_dir}"
                        )
    return 0
...
def add_ch9326_trigger_args(parser: argparse.ArgumentParser, default_edge: str = "rising") -> None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    capture_triggered_dual_low_delay = subparsers.add_parser(
        "capture-triggered-dual-combined-low-delay",
        aliases=["capture-auto-interval-dual-combined-low-delay"],
        help="Initialize MLX/TA612 once, continuously capture, and write trigger windows every fixed interval",
    )
    capture_triggered_dual_low_delay.add_argument("--trigger-count", type=int, default=0, help="Number of triggers to capture; 0 means until stopped")
    capture_triggered_dual_low_delay.add_argument("--trigger-interval", type=float, default=5.866, help="Seconds between automatic trigger events; default: 5.866")
    capture_triggered_dual_low_delay.add_argument("--capture-seconds", type=float, default=1.0, help="Window duration after each trigger")
    capture_triggered_dual_low_delay.add_argument("--warmup-seconds", type=float, default=1.0, help="Seconds to capture before accepting triggers")
    capture_triggered_dual_low_delay.add_argument("--start-when-tasi-above", type=float, default=None, help="TA612 threshold temperature before starting auto triggers")
    capture_triggered_dual_low_delay.add_argument("--start-when-tasi-direction", choices=("rising", "falling"), default="rising", help="Use rising to start when temperature reaches the threshold, or falling to start after it has been above threshold and falls to it; default: rising")
    capture_triggered_dual_low_delay.add_argument("--start-when-tasi-channel", type=int, choices=(1, 2, 3, 4), default=3, help="TA612 channel used by --start-when-tasi-above; default: 3")
    capture_triggered_dual_low_delay.add_argument("--start-when-tasi-timeout", type=float, default=0.0, help="Maximum seconds to wait for the TA612 threshold; 0 means wait forever")
    capture_triggered_dual_low_delay.add_argument("--left-mlx-port", default=None, help="Left MLX USB2UART serial port; default: first detected USB2UART")
    capture_triggered_dual_low_delay.add_argument("--right-mlx-port", default=None, help="Right MLX USB2UART serial port; default: second detected USB2UART")
    capture_triggered_dual_low_delay.add_argument("--left-channel", default="left", help="Left MLX channel name used in output files")
    capture_triggered_dual_low_delay.add_argument("--right-channel", default="right", help="Right MLX channel name used in output files")
    capture_triggered_dual_low_delay.add_argument("--mlx-baud", type=int, default=DEFAULT_BAUD, help=f"MLX USB2UART baud rate, default: {DEFAULT_BAUD}")
    capture_triggered_dual_low_delay.add_argument("--mlx-timeout", type=float, default=2.0, help="MLX serial read/write timeout seconds")
    capture_triggered_dual_low_delay.add_argument("--address", type=lambda s: int(s, 0), default=DEFAULT_MLX_ADDRESS, help="MLX90640 I2C address on each independent bus")
    capture_triggered_dual_low_delay.add_argument("--i2c-rate", type=parse_i2c_rate, default=I2C_RATE_1M, help="MLX I2C rate: 400k, 600k, 800k, 1m, or numeric code")
    capture_triggered_dual_low_delay.add_argument("--stretch", type=int, default=DEFAULT_I2C_STRETCH, help="MLX I2C clock stretch cycles")
    capture_triggered_dual_low_delay.add_argument("--read-chunk-words", type=int, default=DEFAULT_MLX_READ_CHUNK_WORDS, help="MLX register read chunk size in words")
    capture_triggered_dual_low_delay.add_argument("--read-mode", choices=READ_MODES, default=DEFAULT_READ_MODE, help="MLX register read path")
    capture_triggered_dual_low_delay.add_argument(
        "--refresh-rate-hz",
        type=parse_refresh_rate_hz,
        default=DEFAULT_REFRESH_RATE_HZ,
        help="MLX90640 refresh rate Hz: 0.5, 1, 2, 4, 8, 16, 32, or 64; default: 8",
    ...
    capture_triggered_dual_low_delay.add_argument(
        "--left-refresh-rate-hz",
        type=parse_refresh_rate_hz,
        default=None,
        help="Left MLX90640 refresh rate Hz; default: use --refresh-rate-hz",
    )
    capture_triggered_dual_low_delay.add_argument(
        "--right-refresh-rate-hz",
        type=parse_refresh_rate_hz,
        default=None,
        help="Right MLX90640 refresh rate Hz; default: use --refresh-rate-hz",
    )
    capture_triggered_dual_low_delay.add_argument("--startup-delay", type=float, default=DEFAULT_STARTUP_DELAY_SECONDS, help="Delay after MLX I2C setup")
    capture_triggered_dual_low_delay.add_argument("--poll-interval", type=float, default=0.002, help="MLX data-ready poll interval seconds")
    capture_triggered_dual_low_delay.add_argument("--max-polls", type=int, default=2000, help="Maximum MLX polls per subpage")
    capture_triggered_dual_low_delay.add_argument("--emissivity", type=float, default=DEFAULT_EMISSIVITY, help="Object emissivity")
    capture_triggered_dual_low_delay.add_argument("--native-library", default=None, help="Path to libMlx90640Native.dylib")
    capture_triggered_dual_low_delay.add_argument("--tasi-port", default=None, help="TA612C serial port, default: CH340 /dev/cu.usbserial*")
    capture_triggered_dual_low_delay.add_argument("--tasi-baud", type=int, default=DEFAULT_TASI_BAUD, help="TA612C serial baud rate")
    capture_triggered_dual_low_delay.add_argument("--tasi-timeout", type=float, default=0.2, help="TA612C serial timeout seconds")
    capture_triggered_dual_low_delay.add_argument("--tasi-poll-interval", type=float, default=1.0, help="Seconds between TA612 realtime commands")
    capture_triggered_dual_low_delay.add_argument("--tasi-read-size", type=int, default=64, help="TA612C serial read chunk size")
    capture_triggered_dual_low_delay.add_argument("--tasi-stop-first", action="store_true", help="Send TA612 stop command and flush before starting")
    capture_triggered_dual_low_delay.add_argument("--tasi-stop-on-exit", action=argparse.BooleanOptionalAction, default=True, help="Send TA612 stop command before closing")
    capture_triggered_dual_low_delay.add_argument("--tasi-command-delay", type=float, default=0.1, help="Delay after TA612 stop-first command")
    capture_triggered_dual_low_delay.add_argument("--tasi-accept-alt-header", action="store_true", help="Also accept 0x55AA host-order TA612 header while debugging")
    capture_triggered_dual_low_delay.add_argument("--capture-root", default="captures", help="Output root directory")
    capture_triggered_dual_low_delay.add_argument("--print-every", type=int, default=32, help="Print every N MLX frames per channel")
    capture_triggered_dual_low_delay.add_argument(
        "--quiet-live",
        action="store_true",
        help="Only print trigger time and latest temperature summaries; detailed data is still written to CSV/bin files",
    )
    capture_triggered_dual_low_delay.add_argument("--debug-mlx-wire", action="store_true", help="Print raw MLX USB2UART protocol bytes")
    capture_triggered_dual_low_delay.add_argument("--debug-tasi-wire", action="store_true", help="Print raw TA612 serial TX/RX bytes")
    capture_triggered_dual_low_delay.set_defaults(func=cmd_capture_triggered_dual_combined_low_delay)
